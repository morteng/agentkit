...
def test_unannotated_tool_is_hot():
    specs = [_spec("REDACTED.search")]
    plane = _plane({})
    out = plane.resolve(ToolContext(role="editor", role_rank=1), specs)
    assert [s.name for s in out] == ["REDACTED.search"]
    assert plane.rationale["REDACTED.search"].tier == "hot"


def test_discoverable_baseline_is_dropped_from_array():
    specs = [_spec("REDACTED.csg_subtract")]
    plane = _plane({"REDACTED.csg_subtract": ToolVisibility(baseline="discoverable")})
    out = plane.resolve(ToolContext(role="editor", role_rank=1), specs)
    assert out == []
    assert plane.rationale["REDACTED.csg_subtract"].tier == "discoverable"
...
def test_page_match_promotes_discoverable_to_active():
    ...
    plane = _plane(
        {
            "REDACTED.csg_subtract": ToolVisibility(
    ...
    assert [s.name for s in out] == ["REDACTED.csg_subtract"]
    assert plane.rationale["REDACTED.csg_subtract"].tier == "active"
    assert "page match" in plane.rationale["REDACTED.csg_subtract"].reason


def test_min_role_hard_gate_hides_tool():
    specs = [_spec("REDACTED.prune_facts")]
    plane = _plane({"REDACTED.prune_facts": ToolVisibility(baseline="hot", min_role="admin")})
    ctx = ToolContext(role="editor", role_rank=1)
    out = plane.resolve(ctx, specs)
    assert out == []
    assert plane.rationale["REDACTED.prune_facts"].tier == "hidden"
...
def test_intent_keyword_whole_word_match():
    ...
    assert [s.name for s in plane.resolve(hit, specs)] == ["REDACTED.csg_subtract"]
...
def test_discovered_set_promotes_to_active():
    ...
    assert [s.name for s in plane.resolve(ctx, specs)] == ["REDACTED.csg_subtract"]


def test_pluggable_rule_overrides_declarative():
    specs = [_spec("REDACTED.prune_facts")]
    rules = {"prune_facts": lambda ctx: ToolDecision("hidden", "rule: too few facts")}
    plane = _plane({"REDACTED.prune_facts": ToolVisibility(baseline="hot")}, rules=rules)
    out = plane.resolve(ToolContext(role="admin", role_rank=2), specs)
    assert out == []
    assert "rule:" in plane.rationale["REDACTED.prune_facts"].reason


def test_active_cap_enforced():
    specs = [_spec(f"REDACTED.t{i}") for i in range(40)]
...
def test_hot_tier_not_truncated():
    ...
    specs = [_spec(f"REDACTED.h{i}") for i in range(40)]
...
def test_search_tool_always_kept():
    specs = [_spec("kit.search_tools"), _spec("REDACTED.x")]
